...
switch type
    case 'SW'
        ...
        Info.Parameters.Ref_AmplitudeAbsolute = 5;
        ...
        % channel detection
        % ^^^^^^^^^^^^^^^^^        
        Info.Parameters.Channels_Correlate2 = 'all'; % which canonical wave (main, mean, or all)
        Info.Parameters.Channels_Detection = 'threshold'; % correlation/threshold
        Info.Parameters.Channels_Threshold = 0.5;             % amount to adjust threshold if using threshold method
        Info.Parameters.Channels_ClusterTest = true;
        Info.Parameters.Channels_WinSize = 0.100;

        % travelling parameters
        % ^^^^^^^^^^^^^^^^^^^^^        
        Info.Parameters.Travelling_GS = 5; % size of interpolation grid
        Info.Parameters.Travelling_MinDelay = 5; % minimum travel time (ms)
    ...
    case 'SS'
        ...
        Info.Parameters.Ref_AmplitudeAbsolute = [3 3 0.5 3];
        ...
        Info.Parameters.Filter_hPass=11;
        Info.Parameters.Filter_lPass=15;

        Info.Parameters.Channels_Method = 'power'; % wavelet or power (FFT) method
        Info.Parameters.Channels_WinSize = 0.250; % window around ref spindle to search around (in seconds)
