...
for ref_wave = 1 : size(Data.SSRef, 1)
    ...
    for n = 1 : length(spindle_start)
        ...
        if ref_wave > 1
            ...
            if length(SS_indice) > 1
                ...
                SS(earlier_spindle).Ref_Channel = unique([SS(SS_indice).Ref_Channel]);
            ...
            if ~isempty(SS_indice)
                ...
                if peak2peak > SS(SS_indice).Ref_Peak2Peak
                    ...
                    SS(SS_indice).Ref_Channel          =      [ref_wave, SS(SS_indice).Ref_Channel];
                    ...
                    SS(SS_indice).Ref_Channel(end + 1) = ref_wave;
                end
        ...
        SS(SS_count).Ref_Channel          =      ref_wave;
    ...
    %else
        fprintf(1, 'Information: %d spindle bursts added from channel %d \n', length(SS) - original_count, ref_wave);
    %end
